...
class IncomingLetterController extends Controller
{
    ...
    /**
     * Menandai surat sebagai Selesai (Metode Manual).
     *
     * @param string $id
     * @return RedirectResponse
     */
    public function markAsComplete(string $id): RedirectResponse
    {
        try {
            // 1. Lakukan pencarian surat secara manual menggunakan ID dari URL.
            //    Fungsi findOrFail() akan otomatis menampilkan error 404 jika ID tidak ada.
            $letter = Letter::findOrFail($id);

            // 2. Update status pada surat yang sudah ditemukan.
            $letter->status = Letter::STATUS_SELESAI;
            $letter->save();

            return redirect()
                ->back()
                ->with('success', 'Status surat telah berhasil diperbarui menjadi Selesai.');
        } catch (\Throwable $exception) {
    ...
    public function updateStatus(Request $request, Letter $letter): RedirectResponse
    {
        ...
        $request->validate([
            'status' => 'required|in:' . implode(',', [
                Letter::STATUS_KABIRO,
                Letter::STATUS_KABAG,
                Letter::STATUS_SELESAI,
            ]),
        ]);
